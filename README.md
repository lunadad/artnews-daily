...
매일 07:40 KST에 로컬 헤르메스 잡이 GitHub Actions 수집 워크플로를 실행하고(맥이 꺼져 있으면 예약 실행이 대신), 수집 결과가 `data/`에 커밋됩니다. 이 커밋이 Vercel 재배포를 시작하며, 헤르메스의 09:00 KST 카리나 작업은 별도 사이드카 JSON을 `data/karina/`에 푸시합니다. 일별 데이터는 최근 7일만 보관하고 그보다 오래된 파일은 수집 과정에서 정리합니다.
