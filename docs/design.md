...
5. **Stage 5 — 최종 점수·다양성**: 이미지 보너스를 포함해 다시 점수화하고, 카테고리별 최대 2건을 적용한다. 한 피드의 독점을 막기 위해 가능한 경우 서로 다른 매체 도메인 3개를 먼저 확보한 뒤 점수순으로 5건을 채운다.
...
**스코어** (클러스터 단위):
   ...
   | 키워드 시그널 (합산 상한 20) | 0–20 | `record`/`million`/`sold for`/`sotheby`/`christie`/`phillips` +8, `museum director`/`appointed`/`resigns` +6, `restitution`/`repatriat`/`looted` +7, `biennale`/`documenta`/`venice` +6, `lawsuit`/`fraud`/`forgery` +6, `retrospective`/`major exhibition` +5 |
...
국내 점수는 기존 제목 클러스터링·신선도·카테고리 다양성 규칙을 재사용한다. 보도 매체 수는 `min(n,5) × 12`, 매체 신뢰도는 연합뉴스·뉴시스·뉴스1 25점, 종합지·방송 23점, 경제지 21점, 미술 전문지 18점, 기타 8점이다. 국내 키워드는 경매·최고가 8점, 인사·비엔날레·환수 7점, 미술시장·위작·소송 6점, 회고전 5점을 부여하며 합계 상한은 20점이다.
