...
6. **Stage 6 — 번역**: `DEEPL_API_KEY`가 있으면 최종 top5의 제목과 요약만 DeepL 1회 배치 요청으로 한글 번역한다. 없으면 영문 그대로 두고 헤르메스 잡이 사후 번역한다. 제목 반복뿐인 요약은 빈 문자열로 둔다.
...
| 번역 실패 | `titleKo = titleOriginal` (원문 노출) → 헤르메스 잡이 사후 번역. 번역 서비스가 모두 실패하면 텔레그램 경고(하루 1회), 다음 날 수집의 `verify` 단계가 실패해 GitHub 알림 메일 발송 |
