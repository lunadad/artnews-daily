...
3. **번역은 DeepL API Free(`api-free.deepl.com`, 월 50만 자)를 사용한다.** 키는 GitHub Secret `DEEPL_API_KEY`로 주입한다. 최초에는 무인증 `translate.googleapis.com` (client=gtx)를 썼으나 2026-08 말부터 모든 요청이 HTTP 429로 차단되어 교체했다(2026-09-12). 실패 시 **원문을 그대로 노출**하고 파이프라인은 계속 진행하되(fail-soft), 수집 후 `scripts/verify-daily.ts`가 워크플로를 실패로 표시해 알린다.
...
4. **Stage 4 — 상위 12개 보강**: 상위 12개 클러스터 대표만 Google RPC 링크 해석과 기사 페이지 이미지 추출을 수행한다. RSS 요약이 제목 반복(Google News의 `<제목> <매체명>`)이면 같은 페이지의 `og:description`으로 교체한다. 번역은 아직 하지 않는다.
5. **Stage 5 — 최종 점수·다양성**: 이미지 보너스를 포함해 다시 점수화하고, 시장·경매(`market`)는 최대 3건, 나머지 카테고리는 최대 2건을 적용한다. 한 피드의 독점을 막기 위해 가능한 경우 서로 다른 매체 도메인 3개를 먼저 확보한 뒤 점수순으로 5건을 채우며, 한 도메인은 최대 2건(`MAX_STORIES_PER_DOMAIN`)이다. 후보가 모자라면 카테고리 한도 → 도메인 한도 순으로 완화한다.
6. **Stage 6 — 번역**: 최종 top5의 제목과 요약만 DeepL 1회 배치 요청으로 한글 번역한다. 제목 반복뿐인 요약은 빈 문자열로 둔다.
...
| 번역 실패 | `titleKo = titleOriginal` (원문 노출). push 후 `verify` 단계가 실패해 GitHub 알림 메일 발송 |
...
국내 클러스터링은 국제용 공백 단어 Jaccard를 사용하지 않는다. 제목의 구두점을 공백으로 바꾸고 `682억원`·`120만원`을 `682억`·`120만`으로 정규화한 뒤, 2자 이상 토큰에서 `미술관`·`전시`·`작가`·`경매`·`낙찰`·`올해`·`개최` 등 미술 기사 일반어를 제거한 **핵심 토큰 시그니처**를 만든다. 두 토큰이 완전히 같거나, 양쪽 모두 2자 이상이면서 한쪽이 다른 쪽의 부분문자열이면 일치로 센다. 교집합은 A에서 B에 이 조건으로 매칭되는 토큰 수이며, `교집합 / (|A| + |B| - 교집합)`이 `0.15` 이상일 때 같은 사건으로 묶는다. 둘 중 하나라도 비어 있으면 유사도를 0으로 보아 병합하지 않는다. 또한 이미지 보강 뒤 **등록가능 매체 도메인이 같고 null이 아닌 `image` URL이 완전히 같으면** 제목 유사도와 무관하게 같은 사건으로 재병합한다. `coverage`는 이 클러스터 안의 등록가능 매체 도메인 집합 크기다. `qualityCoverage`는 그중 국내 소스 가중치가 18 이상인 서로 다른 등록가능 도메인 수이며, 지역 보도자료 신디케이션을 중요도 신호로 오인하지 않도록 점수에는 이 값만 사용한다.
...
2026-08-05 보관 국내 제목 20건(190쌍)을 부분문자열 허용 방식으로 다시 전수 측정했다. 동아일보의 `미술자료`↔`자료`, `디지털화`↔`디지털` 변형은 0.077에서 0.273으로, 추경호 대구시장 미술관 방문 보도는 0.182로 측정돼 실제 같은 사건만 임계값을 넘었다. 동일 제목은 1.000이었고 그 외 모든 쌍은 0.10 미만이었다. 동일 이미지 쌍은 동아일보 오병합 건 1쌍뿐이었으며, 다른 사건 오탐은 없었다.
...
| 국내 소스 신뢰도 (클러스터 최대값) | 0–26 | 주요 종합·경제일간지 26점, 통신사 24점, 방송 20점, 미술 전문지 18점, 기타 8점 |
| 신선도 | 0–20 | 국제 점수와 동일 |
...
| 경제일간지 | 26 | `mk.co.kr`, `hankyung.com`, `mt.co.kr`, `sedaily.com` |
...
실선정 회귀 테스트는 동일 신선도·카테고리 조건에서 조선일보(26)가 기타 지역지(8)보다 앞서고, 주요 일간지(26) > 통신사(24) > 방송(20) 순서를 유지하는지 확인한다. 또한 조선일보 단독 보도는 저품질 지역지 3곳의 동일 보도(`coverage=3`, `qualityCoverage=0`)보다 앞선다. 후자는 품질 매체가 없는 신디케이션에 coverage 보너스를 주지 않고 −15점을 적용하는 규칙과 함께 검증한다.
...
| 2 | DeepL 키 누락·무효(403)·월 한도 초과(456) | fail-soft — 원문 노출, 파이프라인 중단 없음. `verify` 단계가 워크플로를 실패로 표시 |
