...
export function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 border-b border-border/80 bg-surface/90 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-15 sm:h-16 flex items-center gap-8">
          ...
          <nav className="ml-auto hidden items-center gap-1 sm:flex" aria-label="주요 메뉴">
            {NAV_ITEMS.map((item) => {
              const active = item.href === "/archive" ? pathname.startsWith("/archive") : item.href === "/" && pathname === "/";
      ...
      <nav className="mobile-bottom-nav fixed inset-x-0 bottom-0 z-40 grid grid-cols-3 border-t border-border/80 bg-surface/94 px-2 pt-1.5 backdrop-blur-xl sm:hidden" aria-label="모바일 주요 메뉴">
        {NAV_ITEMS.map((item) => {
          const active = item.href === "/archive" ? pathname.startsWith("/archive") : item.href === "/" && pathname === "/";
