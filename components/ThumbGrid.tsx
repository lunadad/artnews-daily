...
export function ThumbGrid({ items, now }: { items: NewsItem[]; now: string }) {
  const orderedItems = orderThumbGridItems(items);
  return <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 sm:grid-rows-2 sm:gap-3 sm:min-h-[430px]">{orderedItems.map((item, index) => <ThumbTile key={item.id} item={item} relativeTime={formatRelativeTime(item.publishedAt, now)} featured={index === 0} eager />)}</div>;
}
